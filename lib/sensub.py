      ...
      if zipfile.is_zipfile(target):
        ...
        with zipfile.ZipFile(target) as z: # z.extractall(folder) is broken because it doesn't retain the mtime! Workaround:
          for i in z.infolist():
            ...
            if l>259:
              pathTooLong = True
              notify("%s: Path too long having %d characters (limit is 259 characters incl. drive letter and directory separators), ZIP item skipped (please extract manually)." % (rp,l), 1) # Windows limit, excl. final null character.
            else:
